...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python -W ignore run.py train --train-src=./en_es_data/train.es --train-tgt=./en_es_data/train.en --dev-src=./en_es_data/dev.es --dev-tgt=./en_es_data/dev.en --vocab=vocab.json --cuda --log-every=100
elif [ "$1" = "test" ]; then
