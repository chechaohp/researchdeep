...
def forward_softmax(x):
    ...
    Important Note: You must be careful to avoid overflow for this function.
    Functions like softmax have a tendency to overflow when very large numbers
    like e^10000 are computed. You will know that your function is overflow
    resistant when it can handle input like: np.array([[10000, 10010, 10]])
    without issues.
    ...
    Returns:
        A 1d numpy float array containing the softmax results of shape # class
    """
    x = x - np.max(x, axis=0)
    exp = np.exp(x)
...
def backward_softmax(x, grad_outputs):
    ...
    grad_outputs is the gradient of the loss with respect to the outputs of the
    softmax.
    ...
    Returns:
        A 1d numpy float array of the same shape as x with the derivative of the
    ...
    s = forward_softmax(x)
    ds = -s.reshape(-1, 1) @ s.reshape(1, -1)
...
def backward_relu(x, grad_outputs):
    ...
    Args:
        ...
        grad_outputs: A numpy array of the same shape of x containing the
        gradient of the loss with respect to the output of relu

    Returns:
        A numpy array of the same shape as x containing the gradients with
        respect to x.
    """
    # *** START CODE HERE ***
    
    y = (x >= 0).astype(int) * grad_outputs
...
def get_initial_params():
    ...
    This function should return a dictionary mapping parameter names to numpy
    arrays containing the initial values for those parameters.
    ...
    Weight matrices should be initialized with values drawn from a random normal
     distribution. The mean of that distribution should be 0.
    The variance of that distribution should be 1/sqrt(n) where n is the number
    of neurons that feed into an output for that layer.
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    grad_conv_W = np.zeros((conv_channels, 1, conv_width, conv_height))
    for x in range(conv_width):
        for y in range(conv_height):
            for conv_channel in range(conv_channels):
                grad_conv_W[conv_channel, 0, x, y] = np.sum(
                    np.multiply(data[:, x:(x + output_width), y:(y + output_height)], output_grad[conv_channel, :, :]))

    grad_conv_b = np.sum(output_grad, axis=(1, 2))
    
    conv_W_0 = np.zeros((conv_channels, 2 * input_width - conv_width, 2 * input_height - conv_height))
    conv_W_0[:, (input_width - conv_width):input_width, (input_height - conv_height):input_height] = \
        conv_W[:, 0, :, :].reshape(conv_channels, -1)[:, ::-1].reshape(conv_channels, conv_width, conv_height)
    
    grad_X = np.zeros((conv_channels, 2 * input_width - conv_width, 2 * input_height - conv_height))
    for x in range(input_width):
        for y in range(input_height):
            for conv_channel in range(conv_channels):
                grad_X[conv_channel, x, y] = np.sum(
                    np.multiply(conv_W_0[conv_channel, x:(x + output_width), y:(y + output_height)], output_grad[conv_channel, :, :]))

    return grad_X, grad_conv_W, grad_conv_b
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    output_grad is the gradient of the loss with respect to the output of the
    backward max pool layer.
    ...
    for x in range(0, input_width, pool_width):
        for y in range(0, input_height, pool_height):
            for channel in range(input_channels):
                ind = np.unravel_index(np.argmax(data[channel, x:(x + pool_width), y:(y + pool_height)]), (pool_width, pool_height))
                grad[channel, x + ind[0], y+ ind[1]] = output_grad[channel, x // pool_width, y // pool_height]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    grad_w = data.reshape(-1, 1) @ output_grad.reshape(1, -1)
    grad_b = sum(output_grad)
...
def backward_prop(data, labels, params):
    ...
    grad_cross = backward_cross_entropy_loss(y, labels)
    grad_softmax = backward_softmax(logits, grad_cross)
    grad_linear, grad_W2, grad_b2 = backward_linear(W2, b2, flattened, grad_softmax)
    grad_linear = np.reshape(grad_linear, first_max_pool.shape)
    ...
    grad_X, grad_conv_W, grad_conv_b = backward_convolution(W1, b1, data, grad_pool)
...
def nn_train(
    ...
    for batch in range(num_batches):
        ...
        gradient_descent_batch(batch_data, batch_labels, learning_rate, params, backward_prop_func)
...
def main():
    np.random.seed(100)
    train_data, train_labels = read_data('../data/images_train.csv',
